let totalPrice = 0;
const priceCart = document.getElementsByClassName('total-price')[0];
...
function totalCartPrice(price) {
  totalPrice += price;
  priceCart.innerText = totalPrice;
}
...
function cartItemClickListener(event) {
    ...
    const URL = `https://api.mercadolibre.com/items/${event.target.id}`;
    fetch(URL).then((response) => response.json()).then((result) => {
      const removePrice = result.price * (-1);
      totalCartPrice(removePrice);
    });
    saveItemsCart();
  }
}
